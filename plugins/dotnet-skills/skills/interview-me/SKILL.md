...
- The user hasn't said which value they're optimizing for when two reasonable ones are in tension (simplicity vs. flexibility, cost vs. speed, controllers vs. Minimal APIs, Blazor vs. MVC)
...
- **Upstream**: https://github.com/addyosmani/agent-skills/blob/3a6fc6392823e31e2362091bd4e3cddf5b77af14/skills/interview-me/SKILL.md
- **Pinned commit**: `3a6fc6392823e31e2362091bd4e3cddf5b77af14` (synced 2026-06-14)
- **Status**: `modified` (light — pre-code intent-extraction skill that is framework-neutral by nature; upstream body, five-step process, 95% stop test, restate template, rationalization table, and red flags preserved verbatim)
- **Changes**:
  - "When to Use" tension example extended with .NET architectural either/ors (controllers vs. Minimal APIs, Blazor vs. MVC) as concrete cases where the user hasn't named the value being optimized
  - Sibling-skill cross-references (`idea-refine`, `spec-driven-development`, `doubt-driven-development`, `planning-and-task-breakdown`, `source-driven-development`) verified against this plugin's roster — all exist downstream under the same names
  - Preserved: the five-step process, one-question-at-a-time rationale, want-vs-should-want probe, restate template (incl. mandatory "Out of scope"), explicit-yes gate, the 95% confidence stop test, worked example, and the full rationalization/red-flag/verification structure
