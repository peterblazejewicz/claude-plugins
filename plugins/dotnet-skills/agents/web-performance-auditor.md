...
source: vendor/agent-skills/agents/web-performance-auditor.md@3a6fc63
---
...
- **Do not invoke from another persona.** If `code-reviewer` flags a performance concern that warrants a deeper pass, surface that recommendation in the report; the user or a slash command initiates the deeper pass. On Claude Code, subagents cannot spawn other subagents, so the rule is enforced at the platform level. See [`../references/agents-overview.md`](../references/agents-overview.md) for the decision matrix and [`../references/orchestration-patterns.md`](../references/orchestration-patterns.md) for the full pattern catalog.

---

## Source & Modifications

- **Upstream**: https://github.com/addyosmani/agent-skills/blob/3a6fc6392823e31e2362091bd4e3cddf5b77af14/agents/web-performance-auditor.md
- **Pinned commit**: `3a6fc6392823e31e2362091bd4e3cddf5b77af14` (synced 2026-06-14; new persona, added upstream in PR #222)
- **Status**: `modified` (retargeted from generic JS-framework web perf to Blazor / ASP.NET Core; the auditor frame — Quick/Deep modes, the Metric-Honesty Rule, the four review areas, the scorecard, severity taxonomy, and the reviewer-not-implementer composition — preserved)
- **Changes**:
  - Persona reframed as a **.NET Web Performance Engineer** scoped to Blazor (Server / WebAssembly / Auto) and ASP.NET Core MVC / Razor Pages, with an explicit **scope boundary** vs `performance-optimization-dotnet` (server runtime) and `frontend-ui-engineering-avalonia` (desktop XAML, non-web)
  - Quick-mode source globs changed to `.razor` / `.cshtml` / `Program.cs` / `.csproj` / `wwwroot/`
  - Review Scope retargeted: render-mode detection (`@rendermode` Static SSR / Interactive Server / WebAssembly / Auto, .NET 8+); CWV checks add prerender→interactive flicker and SignalR-circuit INP; Loading adds WASM payload (`PublishTrimmed`/`RunAOTCompilation`/globalization data/`BlazorWebAssemblyLazyLoad`), `MapStaticAssets` (.NET 9) fingerprint+compression, `ImportMap`/`@Assets`, `UseResponseCompression`, prerender + `PersistentComponentState` double-fetch; Rendering retargeted to render-mode choice, `<Virtualize>`, `@key`, `ShouldRender`, `StateHasChanged` overcalls, `OnAfterRenderAsync` `firstRender` guard; Network adds `AddOutputCache` (.NET 7+), HTTP/2-3 on Kestrel, EF Core N+1 hand-off
  - AI-generated-pattern bullets rewritten for Blazor/C# (over-broad `@rendermode`, gratuitous `StateHasChanged()`, `async void` handlers, un-debounced `@bind` round-trips, sequential `await` vs `Task.WhenAll`)
  - Tooling table keeps Lighthouse / CrUX / PSI / Chrome DevTools MCP (all operate on the browser-delivered output); browser-MCP cross-reference retargeted from upstream `skills/browser-testing-with-devtools` to `dotnet-skills:integration-testing-dotnet`
  - Rules 8–9 delegate server-runtime concerns and remediation depth to `dotnet-skills:performance-optimization-dotnet` (upstream pointed at `references/performance-checklist.md` + `skills/performance-optimization/SKILL.md`); Composition `agents/README.md` link retargeted to `../references/agents-overview.md` + `../references/orchestration-patterns.md`, and the Claude-Code "subagents can't spawn subagents" platform note added
  - Output-format scorecard "Framework / stack detected" relabeled "Rendering model detected" with .NET examples
  - Metric-Honesty Rule and severity taxonomy preserved verbatim
  - Version anchors (`@rendermode` = .NET 8; `MapStaticAssets` / `ImportMap` / build-time fingerprint+brotli = .NET 9; `AddOutputCache` = .NET 7+) verified against Microsoft Learn at sync time
