...
**`plugin.json` manifest schema is strict.** Valid top-level fields used by this repo (as of 2.5.0): `name`, `version`, `description`, `author`, `license`, `repository`, `homepage`, `keywords`, `category`, `skills`, `commands`. Metadata fields (`author`, `license`, `repository`, `homepage`, `keywords`, `category`) were added in 2.5.0 to feed Copilot CLI's `/plugin` discovery UI — Claude Code ignores them but the schema validator (see [`hesreallyhim/claude-code-json-schema`](https://github.com/hesreallyhim/claude-code-json-schema) — `additionalProperties: false`) accepts them. Other directories Claude Code reads from a plugin — `agents/`, `hooks/`, `.mcp.json` — are **auto-discovered by convention**; do NOT add them as manifest keys (e.g. `"agents": "./agents/"`) or install fails with "invalid manifest file". 2.2.0 shipped broken for this reason; 2.2.1 hotfixed by removing the key.
...
24 engineering workflow skills + 4 .NET-adapted subagents for .NET/C# development (spec → plan → build → verify → review → ship, plus a meta skill). Indirect fork of [addyosmani/agent-skills](https://github.com/addyosmani/agent-skills) (MIT © 2025 Addy Osmani).

Key files:
- `plugins/dotnet-skills/commands/dotnet-skills.md` — slash command (skill + agent + command inventory)
- `plugins/dotnet-skills/commands/{spec,plan,build,test,review,code-simplify,ship}.md` — 7 lifecycle wrappers ported from upstream `.claude/commands/` (added in 2.1.0); `commands/webperf.md` — specialist web-performance audit wrapper (added in 2.6.0)
- `plugins/dotnet-skills/skills/<name>/SKILL.md` — 24 ported skills (added in 2.6.0: `observability-and-instrumentation`, `doubt-driven-development`, `interview-me`)
- `plugins/dotnet-skills/agents/{code-reviewer,security-auditor,test-engineer,web-performance-auditor}.md` — 4 ported subagents (first 3 added in 2.2.0; `web-performance-auditor` added in 2.6.0)
- `plugins/dotnet-skills/agents/{code-reviewer,security-auditor,test-engineer,web-performance-auditor}.agent.md` — Copilot CLI forms of the same 4 personas (first 3 added in 2.5.0; `web-performance-auditor` added in 2.6.0)
...
**GitHub Copilot CLI compatibility (since 2.5.0).** Copilot CLI reads `.claude-plugin/marketplace.json` directly (confirmed live — the `.claude-plugin/` path is deliberate cross-tool compatibility per Copilot's CLI docs). The 24 `SKILL.md` files work on both tools unchanged — the Agent Skills frontmatter (`name` + `description`) is a cross-vendor standard. The 4 agent personas are exposed to Copilot CLI through `.agent.md` wrappers that live alongside the Claude Code `.md` files in `plugins/dotnet-skills/agents/`. **Drift discipline:** bodies must stay in lockstep between each `.md` ↔ `.agent.md` pair; the `.agent.md` footer points to the `.md` sibling as canonical for upstream attribution, so a change to the upstream-sourced body goes in the `.md` file and is re-synced to the `.agent.md` by hand. Custom slash commands don't have a Copilot CLI surface as of 2026-04 ([github/copilot-cli#618](https://github.com/github/copilot-cli/issues/618) open) — `.github/prompts/*.prompt.md` is read only by VS Code Copilot from a *consumer repo's* root, never from inside an installed plugin. Copilot contributors invoke `/agent code-reviewer` / `/agent security-auditor` / `/agent test-engineer` / `/agent web-performance-auditor` and rely on skill auto-matching for the lifecycle workflows. A repo-root [`AGENTS.md`](./AGENTS.md) (added 2.5.0) carries the cross-vendor contributor pointer; Claude Code ignores it.
